...
class Stock extends Model
{
     ...
     */
    protected $fillable = ['title', 'order', 'description', 'active', 'organization_id', 'image'];
    ...
    public $customMessages = [
        ...
        'description.max' => 'Описание не должно превышать 200 символов',
        'active.required' => 'Поле активности обязательно',
    ...
    public function getImage()
    {
        $image = '';
        if(!empty($this->image))
            $image = $this->image->getThumb(300, 132, ['mode' => 'crop']);
        else
            $image = 'https://picsum.photos/300/132';
        return $image;
    }
    ...
    protected $dirtyFields = ['title', 'description'];
...
//                if($exist_relations !== $new_relations) return true;
    ...
    public static function boot(){
        ...
        self::addGlobalScope('notDuplicate', function($query){
            return $query->whereNull('cds_study_stocks.original_id');
        });

        self::addGlobalScope('isActiveStatus', function($query){
            return $query->whereNotIn('cds_study_stocks.status', self::UPDATE_STATUSES);
        });
    }
